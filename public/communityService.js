//Community Service javascript file
...
function handleAcceptClick(){
  ...
  if(reg.test(email) == false) alert("Invalid Email Address!");
    ...
    var requestUrl = "/community-service/" + rsvpIndexClick + "/addPerson";
    ...
    request.addEventListener('load', function(communityService){
      if(communityService.target.status === 200){
        var newRSVP = {
          person: person,
          email: email
        };
        var rsvpHTML = Handlebars.templates.rsvp(newRSVP);
        var container = document.querySelector('.communityService-rsvp-list' + rsvpIndexClick);
        if(container) container.insertAdjacentHTML("beforeend", rsvpHTML);
        closeModal();
      }
      else{
        alert("Error storing RSVP the in database: " + communityService.target.response);
      }
    });
...
function changeMonth(direction){
  ...
  var requestUrlM = "/community-service/" + temp + "/changeMonth";
